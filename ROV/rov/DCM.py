...
class DCM(object):
    ...
    def compensateSensorErrors(self):
        ...
        if len(self.kalmanMagnetomX) == 1000 and len(self.kalmanMagnetomY) == 1000 and len(self.kalmanMagnetomZ) == 1000 and not self.magnetomCal:
            self.magnetomCal = True
            X = []
            Y = []
            Z = []
            for i in range(500):
                self.kalmanMagnetomX.pop(i)
                self.kalmanMagnetomY.pop(i)
                self.kalmanMagnetomZ.pop(i)

            for i in range(len(self.kalmanMagnetomX)):
                X.append(self.kalmanMagnetomX[i])
                Y.append(self.kalmanMagnetomY[i])
                Z.append(self.kalmanMagnetomZ[i])
